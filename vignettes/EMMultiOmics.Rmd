...
`multiOmics` function puts together the first and second stage models and performs the entire algorithm. The output summarizes 1) selected genes from the first stages modeling, 2) coefficients of the clinical features along with their highest posterior density intervals; 3) model performance based on k-fold cross validation. `multiOmics_sensitivity` function performs the same tasks over a range of `a` and `g` values. 

Example with `multiOmics`
```{r eval=FALSE}
M <- GBM_data2$M
G <- GBM_data2$G
fp <- system.file("eg_fx_classification.txt", package="EMMultiOmics")
...
```{r echo=FALSE}
# pre-run results
# eg_multiOmics <- multiOmics_obj
multiOmics_obj <- EMMultiOmics::eg_multiOmics
```
...
Example on `multiOmics_sensitivity`
```{r eval=FALSE}
# params
M <- GBM_data2$M
G <- GBM_data2$G
...
```{r echo=FALSE}
# pre-run results
# eg_multiOmics_sensitivity <- multiOmics_s_obj
multiOmics_s_obj <- EMMultiOmics::eg_multiOmics_sensitivity
```
