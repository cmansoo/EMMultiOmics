...
#' 
#' @export
second_stage <- function(K, R2, n_fold=10, seed=NULL){ #### need to include all of the necessary params for function
  # construct Z matrix
  ...
  for (jjj in 1:n_fold){
    ...
    final_list[[jjj]]=lst # this has to be different from gstr == scale step, so probably two separate functions
    # save(final_list,file=paste0(mypath,"10fold_",a0,"_",gstr,"_",clinic_mod,"_censor.RData"))
  }
  
  #### different step? gstr -- "scale"?? #######
  # what is this?
  
  # `%do%` <- foreach::`%do%`
  # `%:%` <- foreach::`%:%`
  a0_vec <- c(0.1, 1, 10, 50) # can be a user input?
  gstr2 <- "scale"
  for(.a in a0_vec){
    for(jjj in 1:n_fold){
      test_indx <- folds[[jjj]]
      ...
      final_list[[jjj]] <- lst # need to be separate step from above
  ...
  # foreach::foreach(a0=c(0.1, 1, 10, 50)) %:%
  #   foreach::foreach(gstr="scale") %:%
  #   foreach::foreach(jjj=1:n_fold) %do%{
  #     test_indx <- folds[[jjj]]
  #     train_delta <- Delta[-test_indx]
  #     test_delta <- Delta[test_indx]
  # 
